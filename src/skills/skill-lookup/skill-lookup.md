---
name: skill-lookup
description: Activates when the user asks about Agent Skills, wants to find reusable AI capabilities, needs to install skills, or mentions skills for DeltaGravity. Use for discovering, retrieving, and installing skills.
---

When the user needs Agent Skills, wants to extend DeltaGravity's capabilities, or is looking for reusable AI agent components, use the prompts.chat MCP server.
...
- Mentions extending DeltaGravity's capabilities with skills
...
2. Create the directory `src/skills/{slug}/`
3. Save each file to the appropriate location using `create_tool` if it's executable TS, or just acknowledge it if you can't save generic files easily, OR strictly follow these instructions if you have file editing tools:
   - `SKILL.md` → `src/skills/{slug}/SKILL.md`
   - Other files → `src/skills/{slug}/{filename}`
